...
impl ThreadContext {
    ...
    pub fn lock_until_ge(val: &AtomicUsize, target: usize) {
        if target > 0 {
            ...
            alloc::sync::atomic::fence(Ordering::SeqCst);
...
impl ParallelIslandSolver {
    ...
    pub fn init_and_solve<'s>(
        ...
        for _ in 0..num_task_per_island {
            ...
            let velocity_solver =
                alloc::sync::atomic::AtomicPtr::new(&mut self.velocity_solver as *mut _);
            let bodies = alloc::sync::atomic::AtomicPtr::new(bodies as *mut _);
            let multibodies = alloc::sync::atomic::AtomicPtr::new(multibodies as *mut _);
            let manifolds = alloc::sync::atomic::AtomicPtr::new(manifolds as *mut _);
            let impulse_joints = alloc::sync::atomic::AtomicPtr::new(impulse_joints as *mut _);
            let parallel_contact_constraints =
                alloc::sync::atomic::AtomicPtr::new(&mut self.parallel_contact_constraints as *mut _);
            let parallel_joint_constraints =
                alloc::sync::atomic::AtomicPtr::new(&mut self.parallel_joint_constraints as *mut _);
